...
All 5 original build phases (see "Build phases" below) are implemented and validated end-to-end with real dubbing, including the video-trim feature (§ Frontend) and friendly error handling / logging (§ Quality bar). The original architecture spec (`docs/youclone-arquitetura.md`, in Portuguese) is kept as historical reference; this file is the up-to-date source of truth. See `docs/ARQUITETURA.md` (design/why) and `docs/FLUXO.md` (request trace/how) for the current, detailed documentation. Google Cloud TTS was implemented earlier and then **removed** by explicit operator decision (2026-09) — do not re-add it unless asked.
...
**The operator's speaker count is a CAP, never an exact number.** Tela 2's "Pessoas falando" selector feeds `speaker_count` down to pyannote, and it used to arrive as `num_speakers=N` — a hard constraint. That is a trap: asked for 3 speakers on audio where only one person talks, pyannote cannot refuse, so it splits **the same voice** into 3 artificial clusters (off pitch drift, breathing, background music), and `dub._voice_for()` dutifully gives each cluster a different pool voice. The narrator would come out dubbed in three voices, switching mid-video, one of them a different gender. It now passes `min_speakers=1, max_speakers=N` instead, so an over-estimate costs nothing — the model may still conclude there is one speaker. **The two errors are not symmetric and the code must stay on the safe side of that:** guessing too high used to ruin the short, while guessing too low (say 1 when there are 3) merely dubs everyone in the same voice, which is an ordinary single-narrator dub. A second guard sits in `assign_speakers`: any speaker holding fewer than `max(2, 10%)` of the assigned lines is absorbed into the dominant one, because a cluster that small is almost always an artifact — a breath, a music passage — and swapping the video's voice for two seconds reads as a defect, not as a second person. Note what the share guard does *not* catch: a single voice split into evenly sized clusters. Nothing downstream can detect that; not forcing the split in the first place is the actual fix.
...
- **`speed` is advertised API-wide but not honoured by every model.** Same sentence, `speed` absent / 1.3 / 0.8: Gemini returned 5.40s / 5.68s / 5.80s (**ignores it** — the spread is the model's own non-determinism), Grok sped up but would not slow down, and only Kokoro responded correctly in both directions. `_SPEED_CAPABLE_MODELS` is therefore an allowlist, deliberately conservative: asking a model that ignores `speed` would silently leave the line out of time.
  **Only 3 of the 10 pooled voices honour it** — `pm_alex`, `pm_santa` and `pf_dora`, the first three entries of `_voice_pool()`. They lead the pool **because of `speed`, not because of timbre**: the operator picked them after hearing the others drift — without resynthesis the dubbed line runs long and falls behind its caption. That is the same effect measured in "Current state" (max drift from the picture: **0.31s with Kokoro vs 1.50s — the cap — with Gemini**). Do not reorder the pool by how the voices sound; the order is carrying a timing guarantee.
  **There is no fourth.** Kokoro encodes the language in the voice prefix (`p` = Portuguese, `pf_` female / `pm_` male), and probing `/audio/speech` directly shows the pt-BR catalogue is exactly those three: `pf_maria`, `pm_joao`, `pf_ana`, `pm_pedro`, `pf_clara` and `pm_bruno` all return HTTP 400, while `af_heart`/`am_adam` (American English) return 200, which confirms the probe rather than a bad guess at names. So "add another Kokoro voice" is **not** an available fix — the model has no more Portuguese voices to give.
  From the 4th speaker on the pool therefore hands out Gemini/Grok voices, resynthesis is unavailable and time-fitting falls back to `atempo`. The operator accepts this ("caso precise usar mais, pode usar as outras"), but it is a known, already-observed degradation, not a theoretical one: those are exactly the voices that drifted against the captions. Expect a 4+-speaker job to sit less tightly than a single-speaker one. If that ever matters, the options are another model measured to honour `speed` in pt-BR, or Kokoro voices from another language speaking Portuguese (accent likely gives it away) — not a fourth pt-BR Kokoro voice, which does not exist.
...
Relevant config: `DUB_MODEL`/`DUB_VOICE` (default `hexgrad/kokoro-82m` / `pm_alex` — `Algieba` was the first pick by ear and is still in the pool, but the default moved to Kokoro once `speed` decided it), `DUB_VOICE_POOL` (extra `model|voice` entries for multi-speaker; may mix models), `DUB_MAX_SPEEDUP` (1.3), `DUB_KEEP_MUSIC` (bool — turns source separation on), `DUB_MUSIC_DB` (bed level, by ear), `DUB_SEPARATION_JOBS` (4; see "Current state" for why more is slower), `HF_TOKEN`/`DUB_ENABLE_DIARIZATION`.
